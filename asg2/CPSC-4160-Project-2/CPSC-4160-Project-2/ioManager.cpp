...
inputString("")
{
    if (screen == NULL) {
        throw
        std::string("Unable to set video mode; screen is NULL in IOMAnager");
    }
    if ( TTF_Init() == -1 ) {
...
SDL_Surface* IOManager::loadAndSet(const std::string& filename,
...
void IOManager::printMessageAt(const std::string& msg,
    ...
    if (stext) {
        ...
        throw
        std::string("Couldn't allocate text sureface in printMessageAt");
    }
}

void IOManager::printMessageCenteredAt( const std::string& msg,
                                       Sint16 y) const {
    SDL_Surface *stext = TTF_RenderText_Blended(font, msg.c_str(), color);
    if (stext) {
        ...
        throw
        std::string("Couldn't allocate text sureface in printMessageCenteredAt");
    }
}

void IOManager::printStringAfterMessage( const std::string& msg,
